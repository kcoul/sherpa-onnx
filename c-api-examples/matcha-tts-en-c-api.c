...
#ifndef EVAL_MATCHA_MODEL_DIR
#define EVAL_MATCHA_MODEL_DIR "./matcha-icefall-en_US-ljspeech"
#endif

#ifndef EVAL_MATCHA_TEST_INPUT
#define EVAL_MATCHA_TEST_INPUT "./test_input.txt"
#endif

#ifndef EVAL_MATCHA_VOCODER
#define EVAL_MATCHA_VOCODER "./vocos-22khz-univ.onnx"
#endif

#ifndef EVAL_MATCHA_OUTPUT_WAV
#define EVAL_MATCHA_OUTPUT_WAV "./generated-matcha-en.wav"
#endif

static void PrintUsage(const char *prog) {
  fprintf(stderr,
          "Usage: %s [--model-dir PATH] [--text-file PATH] [--vocoder PATH] "
          "[--output PATH] [--sid N] [--speed F] [--debug 0|1] [--no-playback]\\n",
          prog);
}
...
int32_t main(int32_t argc, char *argv[]) {
  const char *model_dir = EVAL_MATCHA_MODEL_DIR;
  const char *text_file = EVAL_MATCHA_TEST_INPUT;
  const char *vocoder_path = EVAL_MATCHA_VOCODER;
  const char *output_wav = EVAL_MATCHA_OUTPUT_WAV;
  int32_t sid = 0;
  float speed = 1.0f;
  int32_t debug = 1;
  int32_t playback = 1;
  ...
  if (!SherpaOnnxFileExists(vocoder_path)) {
    fprintf(stderr,
            "Vocoder file not found: %s\\n"
            "Download/put vocoder here or pass --vocoder PATH.\\n",
            vocoder_path);
    free(text);
    return 1;
  }

  snprintf(acoustic_model_path, sizeof(acoustic_model_path), "%s/model-steps-3.onnx",
           model_dir);
  ...
  config.model.matcha.acoustic_model = acoustic_model_path;
  config.model.matcha.vocoder = vocoder_path;
  config.model.matcha.tokens = tokens_path;
